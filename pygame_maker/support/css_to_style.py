...
# BNF for game engine CSS styling

import re
import logging
from pyparsing import Literal, CaselessLiteral, Word, Group, Optional, \
    ZeroOrMore, OneOrMore, Forward, nums, hexnums, alphas, Regex, \
    ParseException, Keyword, Dict, stringEnd, ParseFatalException, \
...
class CSSStyleEntry(object):
    ...
    CLASS_SELECTOR_RE = re.compile("([0-9a-zA-Z_]*)\.([0-9a-zA-Z_]+)")
    PCLASS_SELECTOR_RE = re.compile(":([0-9a-zA-Z-]+)([(]([^)]+)[)])?$")
    ATTR_SELECTOR_RE = re.compile("([0-9a-zA-Z_]*)[[]([0-9a-zA-Z_]+)(([~*^$|]?[=])\"([0-9a-zA-Z_]+)\")?[\]]")
    ATTR_SELECTOR_TABLE = {
        "=": "matches",
        "~=": "contains_word",
        "|=": "starts_with_word",
    ...
    def _collect_id(self, id_match_info, pcls_match_info):
        if len(id_match_info.group(1)) > 0:
            ...
            if not "type" in self.selector_types:
                self.selector_types.add("type")
        self.id_str = id_match_info.group(2)
        if not "id" in self.selector_types:
    ...
    def _collect_class(self, cls_match_info, pcls_match_info):
        ...
        if not "class" in self.selector_types:
    ...
    def _collect_pclass(self, pcls_match_info):
        ...
        if not "pclass" in self.selector_types:
    ...
    def _collect_attr(self, attr_match_info, pcls_match_info):
        ...
        if attr_match_info.group(3) is not None:
            ...
            self.attr_type = "any"
        if not "attribute" in self.selector_types:
...
class ElementPrioritizerTable(object):
    ...
    ATTR_SELECTOR_PRECEDENCE = [
        "matches", "starts_with_word", "contains_word", "starts_with", "ends_with", "contains", "any"
    ]
    ...
    def attribute_match(self, element, attribute_dict):
        matched = False
    ...
    def element_matched(self, element, match_props, **kwargs):
        matched = False
        prop_list = []
        pseudo_class = None
        element_type = None
...
class CSSStyleParser(CSSStyle):
    ...
    def push_value(self, parsestr, loc, toks):
        ...
        """
        self.logger.debug("push_value(<css str>, loc={}, toks={}):".format(loc, toks))
    ...
    def push_ident_list(self, parsestr, loc, toks):
        ...
        :param toks: The tokens supplied by PyParsing
        """
        self.logger.debug("push_ident_list(<css str>, loc={}, toks={}):".format(loc, toks))
    ...
    def push_param_name(self, parsestr, loc, toks):
        ...
        :param toks: The tokens supplied by PyParsing
        """
        self.logger.debug("push_param_name(<css str>, loc={}, toks={}):".format(loc, toks))
    ...
    def push_parameter(self, parsestr, loc, toks):
        ...
        :param toks: The tokens supplied by PyParsing
        """
        self.logger.debug("push_parameter(<css str>, loc={}, toks={}):".format(loc, toks))
    ...
    def push_param_block(self, parsestr, loc, toks):
        ...
        :param toks: The tokens supplied by PyParsing
        """
        self.logger.debug("push_param_block(<css str>, loc={}, toks={}):".format(loc, toks))
    ...
    def missing_or_bad_value(self, s, loc, expr, err):
        substr_len = min(len(s) - loc, 10)
...
def BNF(css_style_obj):
    ...
    * parameter_block :: selector [ ',' selector ]* '{' ( parameter | comment* )+ '}'
    ...
    if bnf is None:
        ...
        pclass = Combine( Word(":", "-"+alphas) + Optional( '(' + ( Word(nums) | identifier ) + ')' ) )
