...
import unittest
from pygame_maker.actors.gui.styles import *
...
class TestWidget(unittest.TestCase):
    ...
    def test_020shorthand_background(self):
        widget_bk1 = WidgetStyle({"background": ("red", "bkg_foo", "repeat-x", "fixed", "left center")})
    ...
    def test_045valid_widths(self):
        ...
        test_str_styles = ("border-top-width", "border-left-width", "border-right-width", "border-bottom-width",
                           "border-top-width")
        test_str_dict = dict(zip(test_str_styles, test_str_values))
        for style_test_key in test_str_dict:
            style = WidgetStyle({style_test_key: test_str_dict[style_test_key]})
    ...
    def test_050valid_border_styles(self):
        test_values = ("none", "dotted", "dashed", "solid", "double", "groove", "ridge", "inset", "outset", "hidden",
                       "inherit", "initial")
    ...
    def test_065valid_background_attachment(self):
    ...
    def test_075valid_background_positions(self):
        test_values = ("left top", "left center", "left bottom", "right top", "right center", "right bottom",
                       "center top", "center center", "center bottom", "4 2", "0% 15%", "10% 5", "2 20%",
                       "inherit", "initial")
        for val in test_values:
        ...
        test_single_answers = ("left center", "center center", "right center", "center top", "center bottom",
                               "4 50%", "10% 50%")
    ...
    def test_100invalid_styles(self):
        with self.assertRaises(WidgetStyleInvalid):
            style = WidgetStyle({"background-color": "#foo"})
        with self.assertRaises(WidgetStyleInvalid):
            style = WidgetStyle({"background-color": "notacolor"})
        with self.assertRaises(WidgetStyleInvalid):
            style = WidgetStyle({"padding-top": "5p"})
        with self.assertRaises(WidgetStyleInvalid):
            style = WidgetStyle({"z-index": "+-5"})
        with self.assertRaises(WidgetStyleInvalid):
            style = WidgetStyle({"background-resource": "bkginvalid"})
