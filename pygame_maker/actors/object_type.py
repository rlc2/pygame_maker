...
class ObjectType(logging_object.LoggingObject):
    ...
    * implement actions
    ...
    def handle_mouse_event(self, in_event):
        ...
        if gl_minfo:
            for inst in self.get_instances():
                inst.execute_action_sequence(in_event)
            ret = True
        return ret
    ...
    def handle_collision_event(self, in_event):
        ...
        if (in_event["type"] == self) and (in_event["instance"] in self.group):
            in_event["instance"].execute_action_sequence(in_event)
    ...
    * blend modes
    ...
    def handle_instance_event(self, in_event):
        ...
        * outside_room
        ...
        if (in_event["type"] == self) and (in_event["instance"] in self.get_instances()):
            in_event["instance"].execute_action_sequence(in_event)
