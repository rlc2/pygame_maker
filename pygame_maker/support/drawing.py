...
class LineProperties(object):
    def __init__(self, start, end, width, color, style):
        self.start = start
        self.end = end
        self.width = width
        self.color = color
...
def draw_line_segments(surface, line_properties):
    ...
    if line_properties.style == "dotted":
        ...
        print("draw_line_segments(): Line style '{}' is not segmented".format(line_properties.style))
        return
    ...
    if sqrd_length <= (seg_len * seg_len):
        ...
        # print("{} line from {} to {}: midpoint gap ({}, {}, {})".format(line_properties.style, line_properties.start, line_properties.end, sqrd_length, sqrd_full_gap_len, seg_count))
        ...
        # print("horizontal {} line from {} to {} ({})".format(line_properties.style, line_properties.start, line_properties.end, seg_count))
        ...
        pygame.draw.line(surface, line_properties.color.color, (line_properties.end.x - seg_len, line_properties.end.y),
            (line_properties.end.x, line_properties.end.y), line_properties.width)
    elif line_slope_x == 0:
        # simple vertical line
        # print("vertical {} line from {} to {} ({})".format(line_properties.style, line_properties.start, line_properties.end, seg_count))
        ...
        while y_seg_posn < line_properties.end.y - seg_len - 1:
            ...
            seg_idx += 1
        # draw the last segment
        pygame.draw.line(surface, line_properties.color.color, (line_properties.end.x, line_properties.end.y - seg_len),
            (line_properties.end.x, line_properties.end.y), line_properties.width)
    else:
        y_intercept = line_properties.start.y - (float(line_slope_y) * line_properties.start.x) / float(line_slope_x)
...
def draw_line(surface, line_start, line_end, width, color, style):
    ...
    if width == 0:
        ...
        return
    line_properties = LineProperties(line_start, line_end, width, color, style)
    if line_properties.style in ("dotted", "dashed"):
        ...
        pygame.draw.line(surface, color.color, (line_start.x, line_start.y), (line_end.x, line_end.y), width)
